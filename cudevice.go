...
import (
	...
	"github.com/hdfchain/gominer/nvml"
	"github.com/hdfchain/gominer/util"
	"github.com/hdfchain/gominer/work"
)
...
func hdfchainCPUSetBlock52(input *[192]byte) {
	if input == nil {
		panic("input is nil")
	}
	C.hdfchain_cpu_setBlock_52((*C.uint32_t)(unsafe.Pointer(input)))
}

func hdfchainHashNonce(gridx, blockx, threads uint32, startNonce uint32, nonceResults cu.DevicePtr, targetHigh uint32) {
	C.hdfchain_hash_nonce(C.uint32_t(gridx), C.uint32_t(blockx), C.uint32_t(threads),
...
func (d *Device) runDevice() error {
	...
	for {
		...
		hdfchainCPUSetBlock52(endianData)
		...
		hdfchainHashNonce(gridx, blockx, throughput, startNonce, nonceResultsD, targetHigh)
