...
extern "C" void hdfchain_hash(void *output, const void *input)
{
...
__global__ void hdfchain_gpu_hash_nonce(const uint32_t threads, const uint32_t startNonce, uint32_t *resNonce, const uint32_t highTarget)
{
...
extern "C" {
DLLEXPORT void
hdfchain_hash_nonce(uint32_t grid, uint32_t block, uint32_t threads,
    uint32_t startNonce, uint32_t *resNonce, uint32_t targetHigh)
{
	hdfchain_gpu_hash_nonce <<<grid, block>>> (threads, startNonce, resNonce, targetHigh);
}
}

extern "C" {
__host__ DLLEXPORT void
hdfchain_cpu_setBlock_52(const uint32_t *input)
{
	/*
